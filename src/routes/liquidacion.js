...
router.get('/consulta/', isLoggedIn, async (req, res) => {
    ...
    var vista_liquidacion = await pool.query('(SELECT fec_ingreso, numdoc, paciente, entidad, eps, cups FROM informe WHERE entidad = ? AND fec_ingreso like ?) UNION (SELECT fec_ingreso, numdoc, paciente, entidad, eps, cups FROM informec WHERE entidad = ? AND fec_ingreso like ?) ORDER BY fec_ingreso ASC', [entidad, yearmes, entidad, yearmes]);
